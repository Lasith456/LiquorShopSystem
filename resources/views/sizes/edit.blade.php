...
<div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 py-10 px-4">
    <div class="bg-white shadow-lg rounded-2xl w-full max-w-2xl p-8">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-indigo-700">✏️ Edit Size</h2>
        ...
        <form action="{{ route('sizes.update', $size->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')

            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Size Label</label>
                <input type="text" name="label" value="{{ old('label', $size->label) }}"
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                          class="w-full border-gray-300 focus:ring-indigo-500 focus:border-indigo-500 rounded-lg shadow-sm text-sm px-4 py-2">{{ old('description', $size->description) }}</textarea>
            </div>

            <div class="text-center">
                <button type="submit" 
                        class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2 px-6 rounded-lg shadow-md transition">
                    <i class="fa-solid fa-floppy-disk mr-2"></i> Update Size
                </button>
            </div>
        </form>
    </div>
