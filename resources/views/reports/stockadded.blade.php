...
<div class="bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen p-6">
    <div class="max-w-7xl mx-auto bg-white rounded-2xl shadow-md p-6">
        ...
        <div class="flex justify-between items-center border-b pb-4 mb-6">
            ...
            <div class="space-x-2">
                <a href="{{ route('reports.stockadded.export.pdf', request()->query()) }}" class="px-3 py-2 bg-red-600 text-white rounded-md hover:bg-red-700">
                    <i class="fa fa-file-pdf mr-1"></i> Export PDF
                </a>
                <a href="{{ route('reports.stockadded.export.excel', request()->query()) }}" class="px-3 py-2 bg-green-600 text-white rounded-md hover:bg-green-700">
        ...
        <form method="GET" id="filterForm" class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6" x-data="filterHandler()">
            <div>
                ...
                <input type="date" name="start_date" value="{{ request('start_date') }}" class="w-full border-gray-300 rounded-md focus:ring-2 focus:ring-indigo-400">
            </div>

            <div>
                <label class="block text-sm text-gray-600">End Date</label>
                <input type="date" name="end_date" value="{{ request('end_date') }}" class="w-full border-gray-300 rounded-md focus:ring-2 focus:ring-indigo-400">
            </div>

            <div>
                <label class="block text-sm text-gray-600">Category</label>
                <select name="category_id" x-model="selectedCategory" @change="filterProducts" class="w-full border-gray-300 rounded-md focus:ring-2 focus:ring-indigo-400">
            ...
            <div>
                ...
                <select name="product_id" id="productSelect" class="w-full border-gray-300 rounded-md focus:ring-2 focus:ring-indigo-400">
                    <option value="">All</option>
                    @foreach ($products as $p)
                        <option value="{{ $p->id }}" data-category="{{ $p->category_id }}" {{ request('product_id') == $p->id ? 'selected' : '' }}>
                            {{ $p->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="md:col-span-4 flex justify-end gap-3 mt-2">
                ...
                @if(request()->has('start_date') || request()->has('end_date') || request()->has('category_id') || request()->has('product_id'))
                    <a href="{{ route('reports.stockadded') }}" class="px-4 py-2 bg-gray-300 text-gray-800 rounded-md hover:bg-gray-400 flex items-center gap-1">
            ...
            <table class="min-w-full border border-gray-200 text-sm">
                ...
                <tbody>
                    ...
                    @foreach ($stockItems as $item)
                        @php $totalCost += $item->total ?? 0; @endphp
                        <tr class="border-b hover:bg-gray-50">
                            ...
                            <td class="px-3 py-2 text-right">Rs. {{ number_format($item->total, 2) }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
...
function filterHandler() {
    return {
        ...
        filterProducts() {
            ...
            // reset selected product if it doesn't belong to the chosen category
