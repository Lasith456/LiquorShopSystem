...
<div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 py-10 px-4">
    <div class="bg-white shadow-lg rounded-2xl w-full max-w-2xl p-8">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-indigo-700">✏️ Edit Category</h2>
        ...
        <form action="{{ route('categories.update', $category->id) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                          class="w-full border-gray-300 focus:ring-indigo-500 focus:border-indigo-500 rounded-lg shadow-sm text-sm px-4 py-2">{{ old('description', $category->description) }}</textarea>
            </div>
